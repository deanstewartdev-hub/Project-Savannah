 ...
 * - Prevent internal errors and implementation details
...
const ScriptsController = (() => {
   ...
   * also be supplied directly instead of inside request.idea.
   *
    ...
    try {
      ...
      return createSuccessResponse_(
        ...
        "Script generated successfully.",
        {
          ...
          persistence:
            clone_(result.persistence || {}),

          ai:
            clone_(result.ai || {}),

          validation:
            clone_(result.validation || {}),

          formatting:
            clone_(result.formatting || {}),
  ...
  function listScripts(request) {
    ...
    try {
      ...
      if (query.ideaId) {
        ...
        scripts =
          ScriptsRepository.getScriptsByStatus(
            query.status
          );
      } else {
        scripts =
   ...
   * - { ideaId: string }
   *
   * @param {*} request Idea ID or request object.
   * @return {Object} Frontend-safe response.
   */
  function getLatestScriptForIdea(request) {
    const requestId =
      createControllerRequestId_();
  ...
  function getMetrics() {
    ...
    try {
      ...
      scripts.forEach(function (script) {
        if (
          statusCounts[script.status] ===
          undefined
        ) {
          statusCounts[script.status] = 0;
        }
   ...
   */
  function normaliseGenerateRequest_(request) {
    if (
      !request ||
    ...
    const sourceIdea =
      request.idea &&
      typeof request.idea === "object" &&
      !Array.isArray(request.idea)
        ? request.idea
        : request;
    ...
    return {
      idea: idea,
      options: sanitiseOptions_(options)
    };
  }
   ...
   */
  function normaliseListRequest_(request) {
    const source =
      request === undefined ||
      request === null
        ? {}
   ...
   */
  function resolveScriptWordCount_(script) {
    const metadataWordCount = Number(
      script.metadata &&
      script.metadata.wordCount
    );

    if (
      isFinite(metadataWordCount) &&
      metadataWordCount >= 0
    ) {
   ...
   */
  function resolveValidationAttempts_(script) {
    const candidate = Number(
      script.metadata &&
      script.metadata.usage &&
      (
        script.metadata.usage.attemptCount ||
        script.metadata.usage
  ...
  function extractScriptId_(request) {
    ...
    const scriptId =
      normaliseOptionalString_(value);
  ...
  /**
   * Resolves the latest script update timestamp.
   *
   * @param {Object[]} scripts Script records.
   * @return {string} ISO timestamp or empty string.
   */
  function resolveLatestUpdatedAt_(scripts) {
    if (
   ...
   *
   * Internal stack traces and source filenames are not
  ...
  /**
   * Converts an internal error into a safe plain object.
   *
   * @param {*} error Internal error.
  ...
  /**
   * Maps error types to stable frontend error codes.
   *
   * @param {string} errorName Error name.
   * @return {string} Error code.
   */
  function resolveErrorCode_(errorName) {
    const codes = {
      ScriptEngineError:
  ...
  /**
   * Removes accidental stack or source details from an
   * error message.
   *
   ...
   */
  function createControllerError_(message) {
 ...
 *
 * These functions are callable from the frontend with:
 *
 * google.script.run
 *   .withSuccessHandler(...)
 ...
 */
function scriptsGenerateScript(request) {
  return ScriptsController.generateScript(
    request
  );
}

/**
 * Generates a script preview without persistence.
 *
 * @param {Object} request Frontend request.
 * @return {Object} Controller response.
 */
function scriptsGeneratePreview(request) {
  return ScriptsController.generatePreview(
    request
  );
}
...
function scriptsList(request) {
  return ScriptsController.listScripts(
    request || {}
  );
}
...
function scriptsGet(request) {
  return ScriptsController.getScript(
    request
  );
}
 ...
 * @return {Object} Controller response.
 */
function scriptsGetLatestForIdea(request) {
  return ScriptsController
    .getLatestScriptForIdea(request);
}

/**
 * Loads script dashboard metrics.
 *
 * @return {Object} Controller response.
 */
function scriptsGetMetrics() {
  return ScriptsController.getMetrics();
}
...
function testScriptsControllerMetrics() {
  ...
  if (
    typeof response.data.totalScripts !==
      "number" ||
    typeof response.data.createdToday !==
      "number"
  ) {
    throw new Error(
